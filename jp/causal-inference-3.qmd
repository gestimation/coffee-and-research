...
::::::::::::::::::::::::::::::::: dialogue
::: voice-father
お父さん「まだ話は続くから、コーヒー淹れなおそうか。ここからの話題はDAGと確率変数の関係について。まず、以下のRコードの実行結果をみてよ」
:::
::::::::::::::::::::::::::::::::: 

::: {.callout-note appearance="simple" collapse="true" title="ピロリ菌と胃がんの因果関係を表すDAG"}

ピロリ菌と胃がんの因果関係を表すDAGをもう一度示します。
...
::: {.callout-note appearance="simple" collapse="true" title="Rパッケージを用いたDAGのコードはこちら"}

このDAGを使って何がわかるかをみるため、Rの**dagitty**パッケージを実行してみます。`dagitty()`は、変数の出口と入口を指定することで、ノードを矢印で結びます。`adjustmentSets()`は、DAGを与えて調整すべき変数の集合（交絡因子の集合）を調べるツールです。
...
coordinates(dag) <- list(x=c(A=1,B=3,C=2,D=3,E=1),y=c(A=1,B=1,C=2,D=3,E=3))
plot(dag)
confounders <- adjustmentSets(dag, exposure = "E", outcome = "D")
print(confounders)
```
:::
...
::::::::::::::::::::::::::::::::: dialogue
::: voice-daughter
私「ふむ。今回も呪文の意味はわからぬが、とりあえず、`dagitty()`の出力を`plot()`に入れるとDAGが描かれた。そして`adjustmentSets()`の出力を`print()`に入れると、交絡因子はBとC、AとCって表示された」
:::

::: voice-father
お父さん「その通り。ここからは、どうやってDAGから交絡因子として調整すべき変数たちを探すかっていうアルゴリズムについの話すね。そのためには同時分布と条件付分布が要る。人によっては復習が必要かもしれない」
:::
::::::::::::::::::::::::::::::::: 

::: {.callout-note appearance="simple" collapse="true" title="確率の復習"}
...
::: voice-father
お父さん「簡単にいうとそういうこと。**合流点があると、変数間の相関関係がトリッキーになる**んだ。簡単にいうと、**dagitty**パッケージは合流点に気をつけながら、バックドアパスをブロックする変数を見つけてくれるんだ」
:::
::::::::::::::::::::::::::::::::: 
...
::: {.callout-note appearance="simple" title="冒頭のDAGについてのクイズです"}

以下のDAGにおいて、交絡因子として調整すべき変数の集合として正しいのは、次のうちどれでしょうか。
...
DAGをみると、Cは合流点でもあり、共通原因にもなっていますよね。したがって、Aだけ、Bだけ、AとBだけを調整すると、パスE←C→Dによる疑似相関が残ってしまいます。さらに、Cは合流点なので、調整するとパスE←A→C←B→Dによる相関が生じてしまいます。

結論をいうと、パスE←A→C←B→Dをブロックするためには、複数の変数を同時に調整する必要があります。すなわち、交絡因子の集合は、CとAまたはCとBです。この変数の組み合わせは`dagitty()`と`adjustmentSets()`の実行結果と同じですよね。
:::
