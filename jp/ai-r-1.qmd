...
::::::::::::::::::::::::::::::::: dialogue
::: voice-daughter
私「`add_p.tbl_cross`のヘルプによるとWilcoxon順位和検定と$\chi^2$検定みたい」
:::

::: voice-father
お父さん「そうそう。今回はそれであってる。期待人数が5より低いと、$\chi^2$検定じゃなくてFisher正確検定が選ばれるけどね。それともうひとつやることがある」
:::
::::::::::::::::::::::::::::::::: 
...
::: {.callout-note appearance="simple" collapse="true" title="Rコードと結果はこちら"}
# Double programming check using tableone
install.packages("tableone")
library(tableone)

# Prepare data with same labeling
df_check <- diabetes.complications %>%
    select(all_of(confounder_names), fruitq1) %>%
    mutate(fruitq1 = factor(fruitq1, levels = c(0, 1), labels = c("High Intake", "Low Intake")))

# Create summary table using tableone
table2 <- CreateTableOne(vars = confounder_names, strata = "fruitq1", data = df_check, test = TRUE)
print(table2, showAllLevels = TRUE)
:::

::::::::::::::::::::::::::::::::: dialogue
::: voice-daughter
私「表の桁数は違うけど、数値はあってたみたい。これでいいってこと？」
:::

::: voice-father
お父さん「患者背景の集計まではね。いくつか注意点を振り返ってもいい？」
:::
...
::: voice-father
お父さん「うんうん。ここまでをまとめると、どんな解析をするときも、この3つだけは予め考えておく」
:::
