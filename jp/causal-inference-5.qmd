...
::: {.callout-note appearance="simple" title="調整変数は手持ちの変数一覧ではない"}
交絡を調整するとき、測定できた変数をすべてモデルに入れればよいわけではありません。どの変数で調整すべきかは、曝露、アウトカム、共通原因、中間因子、合流点の関係から考えます。

DAGは、その判断を図として見えるようにする道具です。有向分離で探しているのは、手元の変数の数ではなく、バイアスのあるパスを閉じ、開いてはいけないパスを開かないための調整変数の組み合わせです。

したがって、調整後の数字は「たくさん調整したから正しい数字」ではありません。どの問いに答えるために、どのパスを閉じる必要があると考え、そのためにどの変数を選んだのか。その説明とセットになってはじめて、調整済みの数字は意味を持ちます。
:::
