---
title: "A Circle, an Equation, and a Cylinder"
description: ""
tags:
  - Causal model
  - Language & writing
  - Probability model
format:
  html:
    toc: true
date: 2025-12-13
---

::: {.hero-header}
![](../image/father-daughter.png){.hero-header-img}
:::

# Causal Inference IV − A Circle, an Equation, and a Cylinder

::: {.serif-block .text-right}
Keywords: causal model, language & writing, probability model
:::

------------------------------------------------------------------------

### 問いの答えは、どの抽象表現で世界を見るかで変わる

::::::::::::::::::::::::::::::::: dialogue
::: voice-daughter
私「お父さんさ、丁寧なのはいいんだけど。合流点の説明ってもっと簡単にできないの？この条件付確率分布の式がきっついわ」
:::

::: voice-father
お父さん「その気持ちはわかるよ、でも式を出さないと正確じゃないと思って」
:::

::: voice-daughter
私「それ自体はありがたいんだけどね。たとえ話とかでも十分そうだけど」
:::

::: voice-father
お父さん「でもさ、最初の質問覚えてる？"確率モデルでもRubin因果モデルでも、交絡の説明が不足しているんじゃないか"、そこが出発点だったよね。だから、合流点を何か別のものに置き換えるだけでは足りないんだ」
:::

::: voice-daughter
私「まあそうだけど」
:::

::: voice-father
お父さん「具体的な式をナプキンを書いたのは理由がある。同じものをDAGと数式の2通りで表現していることを知ってほしかったんだ。抽象的な表現を併用することは意外に多い。ここにマグカップがあるよね」
:::

::: voice-daughter
私「いつもコーヒーを飲んでるやつね」
:::

::: voice-father
お父さん「これを上から見てごらん」
:::

::: voice-daughter
私「ん？こう？」
:::

::: voice-father
お父さん「そう。円が見えるでしょ」
:::

::: voice-daughter
私「そうだね。それがどうかしたの？」
:::

::: voice-father
お父さん「数学の授業で、x軸とy軸をとると円はこんな方程式で表せるって習ったでしょ。DAGと条件付確率の式の関係は、これに似ている」
:::

$$x^2+y^2=r^2$$

::: voice-daughter
私「円は見た目、円の式は数式って意味？」
:::

::: voice-father
お父さん「そう。共通の分析対象を、別の表現で扱ってるでしょ。円の話をするとき、数式が要らなかったら、円の絵を描いた方がはやいよね。DAGもそう。条件付確率の式から、つながり方だけを抜き出した射影なんだ。分布の形やデータの型は捨て、複雑な式をいったん横に置いて必要な構造だけを見るためにね。DAGは、つながり方だけを見るためのトポロジカルな道具なんだ」
:::

::: voice-daughter
私「そりゃあ、矢印でつながってるだけの方が、まずはわかりやすいよ」
:::

::: voice-father
お父さん「じゃあ今度はマグカップを斜めから見てごらん」
:::

::: voice-daughter
私「ん。円が立体になるっていいたいんだよね」
:::

::: voice-father
お父さん「その通り、マグカップが円柱として見える。この円柱はRubin因果モデルの潜在結果変数みたいなものじゃない？上から観測できるデータだけじゃ、全体はわからないよね。それが観測できない潜在結果変数なんだ」
:::

::: voice-daughter
私「ほえー詩人だね」
:::

::: voice-father
お父さん「1つのマグカップの中に、確率モデル、DAG、Rubin因果モデルという3つの構造が隠れている。Rubin因果モデルまでは説明しきれないから、["参考資料"](Causal Inference for Medical Research Ch 2-3 RCM.pdf)をみてほしい。抽象的な構造を使い分けてデータから情報を引き出す。それも、単なる情報処理や圧縮じゃなく、患者さんのための知識を得るためにね。統計解析は人間のためのもの。だからこそ、どの抽象表現で世界を見るかが大事になる」
:::
:::::::::::::::::::::::::::::::::

::: {.callout-note appearance="simple" title="モデルを使い分ける目的"}
DAG、確率モデル、Rubin因果モデルは、同じ現象を同じ言葉で説明する道具ではありません。DAGは変数のつながりと調整の候補を考える道具、確率モデルは観測データの分布や推定方法を記述する道具、Rubin因果モデルは「もし別の治療を選んでいたら」という反事実を定義する道具です。

実際の研究では、まず何を知りたいのか、つまり推定目標（estimand）を言葉にします。そのうえで、どのモデルがどの部分を支えているのかを分けて考えると、解析の目的と仮定が見えやすくなります。

この順番を逆にして、先に手元の変数をモデルに入れてから「調整済みの答え」を探すと、数字だけが独り歩きします。調整済み推定値は、どこかに眠っている真実そのものではありません。問いに対して、どの抽象表現と仮定を選んだかに対応する、ひとつのモデル上の答えです。
:::

### 変数をどう選ぶ

::::::::::::::::::::::::::::::::: dialogue

::: voice-daughter
私「でもね、私にとっては、モデルよりも変数の選び方の方が切実な問題なんだ。ピロリ菌除菌のDAGでいえば、ピロリ菌除菌の有無の他に、胃がん発生、性格、年齢、体質という変数をDAGに入れたでしょ。実際の研究でどうやって選ぶの？」
:::

::: voice-father
お父さん「そこが難しいんだけど、**医学知識**や**研究デザイン**を根拠にもっともらしいDAGを描くしかないね。胃がんリスク因子だとか、ランダム化臨床試験なら治療割付と共変量は独立なはず、だとか」
:::

::: voice-daughter
私「そうするとDAGの変数が無限に増えちゃうと思うんだ。だって胃がんが発生するためには、消化管が発生しないといけないでしょ。そうすると、その前にはミクロレベルでは細胞があり、マクロレベルでは生物がないとおかしいよね。ピロリ菌除菌を受ける原因だって、社会的要因かもしれないし」
:::

::: voice-father
お父さん「ああ、そういう発想もあるね。因果観のなかには、単にある原因と結果を結びつけるだけじゃないものもある。あらゆる物質がNewton力学に従っているようにね」
:::

::: voice-daughter
私「いま物質の話をしてないんだけど」
:::

::: voice-father
お父さん「うん。じゃあ枠組みをもう少し広くしよう。日本語には"因果"と並んで"縁起"という言葉がある。因果は原因と結果を意味するよね。一方の縁起は、あらゆる出来事や認識が重なって、結果が生じるという文脈で使われる。でも、ランダム化臨床試験や観察研究の分析対象は、縁起の文脈より、もっと部分的じゃない？」
:::

::: voice-daughter
私「そうだね。知りたいのは医薬品の有効性やピロリ菌除菌の効果だけだね」
:::

::: voice-father
お父さん「うん。どの枠組みが正しいかっていう話じゃないよ。臨床研究の場合は、DAGに入れる変数は研究仮説や目的に関わると思われるものにフォーカスしてもいいんじゃないかって言いたいわけ。使用目的は、たとえば交絡因子を特定して、推測の妥当性を吟味することだからね」
:::
::::::::::::::::::::::::::::::::: 

::: {.callout-note appearance="simple" title="このエピソードに関係するクイズです"}

DAGはいくつかのアイデアが元になっていますが、そのひとつにSewall Wrightが描いたパスダイアグラムがあります。その最初のものは、1920年にProceeding of the National Academy of Sciencesで発表された論文に載っていたものといわれています。さて、この図は何の因果関係を分析したものでしょうか？

1.  がんとその原因
2.  車の故障とその原因
3.  QOLとその原因
4.  モルモットの特徴とその原因

:::

::: {.callout-note appearance="simple" collapse="true" title="答えはこちら"}
- **正解は4です。**

![](causal-inference-4.png)

:::

### 次のエピソード
- [Backdoor Paths, Block, and d-Separation: A Clue for Adjusting for Bias](causal-inference-5.html)

::: {.callout-note appearance="simple" collapse="true" title="他のエピソードはこちら"}

**このシリーズのエピソード**

- [Three-Variable DAGs: The Smallest Building Blocks of Causal Structure](causal-inference-1.html)
- [A Subtle Distinction between Common Causes and Confounders](causal-inference-2.html)
- [DAGs and Conditional Distributions: Two Languages for the Same Structure](causal-inference-3.html)
- [A Circle, an Equation, and a Cylinder](causal-inference-4.html)
- [Backdoor Paths, Block, and d-Separation: A Clue for Adjusting for Bias](causal-inference-5.html)
- [Volatility, Uncertainty, Complexity, and Ambiguity in Causal Inference](causal-inference-6.html)

**過去のシリーズ**

- [Study Design I](study-design-1.html)
- [Frequentist Thinking I](frequentist-1.html)
- [Frequentist Experiments I](frequentist-4.html)
- [Effects and Time I](effects-1.html)
- [Adjusting for Bias I](logistic-regression-1.html)
- [Truth I](truth-1.html)

**用語集**

- [Statistical Terms in Plain Language](glossary.html)
:::
