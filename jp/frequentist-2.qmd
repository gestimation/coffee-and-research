...
::: {.callout-note appearance="simple" title="よくある読み違い"}
p値は、帰無仮説が正しい確率ではありません。また、p値が小さいことは、その研究結果が科学的・臨床的に重要であることを自動的には意味しません。逆に、p値が大きいことも、「差がない」「同等である」「研究に価値がない」という結論を意味しません。

p値を読むときは、少なくとも、帰無仮説、アウトカム、有意水準、検定方法、中間解析や多重性の扱いをMethodsで確認します。p値は単独の数字ではなく、研究計画と解析ルールの中で意味を持つ数字です。
:::
...
ハザード比を推定する方法はいくつかありますが、もっともポピュラーなのは**survival**パッケージの`coxph()`です。以前のエピソードでは、関数`generate_data()`を用いて、ストーマの有無やOSのデータを生成しました。今回は同じデータにCox回帰を当てはめ、ストーマあり群とストーマなし群のハザード比を計算してみます。`generate_data(hr1, hr2, hr3)`では、死亡ハザード比の真値はhr3という引数で指定できます。
:::
