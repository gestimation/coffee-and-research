...
::: {.callout-note appearance="simple" collapse="true" title="調整CIFのRコードと結果はこちら"}
```{r warning=FALSE}
cifplot(Event(t,epsilon)~fruitq1, data=dat, outcome.type="competing-risk", 
...
::: {.callout-note appearance="simple" collapse="true" title="調整CIFを比較するためのRコードと結果はこちら"}
```{r warning=FALSE}
...
::: voice-father
お父さん「うん。じゃあ振り返りをかねて調整CIFを推定するときの典型的な失敗例を4つ挙げよう。ひとつでも踏むと、図がそれっぽく出ても計算は間違っている。あらかじめ知っておいた方がいい」
:::
...
::: {.callout-tip appearance="simple" title="調整CIFを作るときの実務チェックリスト"}

このシリーズでは、Table 1と調整CIFを作成するためのワークフローを見てきましたが、実務ではその結果が正しいことを自主点検できることが重要です。AIを使っても責任は解析者が担います。実務チェックリストを用意しましたのでご利用ください。
