...
このシリーズは、因果推論の実務フローを見て頂くために、"A Conversation on Causality at Our Table"の付録として用意したものです。全3エピソードの中で、5つのスキルを身に着けられるように構成しています。
...
広く使われているスキルほど、深堀していくとおもしろい概念が潜んでいます。交絡調整ってなにをしているの？因果とデータってどういう関係なの？興味がある方は["A Conversation on Causality at Our Table"](https://gestimation.github.io/coffee-and-research/jp)もあわせてお楽しみください。また、Rパッケージの詳細については、それぞれのVignettes等をご参照ください。
:::
...
::: {.callout-note appearance="simple" collapse="true" title="DAGを描くためのRコードと結果はこちら"}
```{r warning=FALSE}
...
::: {.callout-note appearance="simple" collapse="true" title="データを確認するためのRコードと結果はこちら"}
...
::: voice-father
お父さん「そう、マッチングと重み付けは別。マッチングしたデータセットを作ってから、累積発生曲線を描くと、個人間の対応が無視されてしまう。これがはっきり悪いわけじゃないんだけどね。今回は、ひとりひとりに重みを付けたデータセットを作って、重み付きAalen-Johansen曲線を描く」
:::
...
::: {.callout-note appearance="simple" collapse="true" title="CBPSの重みを計算するためのRコードと結果はこちら"}
...
::: {.callout-note appearance="simple" collapse="true" title="オーバーラップを確認するためのRコードと結果はこちら"}
...
::: {.callout-note appearance="simple" collapse="true" title="共変量バランスを確認するためのRコードと結果はこちら"}
...
::: voice-father
お父さん「そう、点は共変量ごとのバランス度合いを表している。連続データでは**標準化平均差（standardized mean difference）**、2値データでは**割合の差**だね。もちろん"観測された共変量"以外は、バランスがとれたかどうかは確認できないけど、これはDAGが正しいかどうかっていう別の問題になる。共変量バランスが取れて、オーバーラップも極端に崩れていないなら、累積発生曲線を描く段階に進もう」
:::
...
::: voice-father
お父さん「うん、最初は重みなしのただの累積発生曲線を確認した方がいい。これは因果効果じゃなくて、イベント、打ち切り、競合リスクの発生状況を把握するためのもの。年齢や喫煙による交絡が残っているからね」
:::
::::::::::::::::::::::::::::::::: 
...
::: {.callout-note appearance="simple" collapse="true" title="累積発生曲線（デフォルト）のRコードと結果はこちら"}
```{r warning=FALSE}
...
::: {.callout-note appearance="simple" collapse="true" title="競合リスクをマーキングしたRコードと結果はこちら"}
```{r warning=FALSE}
cifplot(Event(t,epsilon)~fruitq1, data=dat, outcome.type="competing-risk", 
...
::: voice-father
お父さん「この2グループは、それぞれ"もともとの集団全体が、high intakeだったときの集団"と"もともとの集団全体がlow intakeだったときの集団"っていうふうに解釈する。もともとの集団ではなく、どちらも仮想的な集団だ。果物摂取量が高いときのアウトカムと、低いときのアウトカムの差が、因果効果なんだ。じゃあ調整累積発生曲線を見てみよう」
:::
::::::::::::::::::::::::::::::::: 

::: {.callout-note appearance="simple" collapse="true" title="調整累積発生曲線のRコードと結果はこちら"}
```{r warning=FALSE}
cifplot(Event(t,epsilon)~fruitq1, data=dat, outcome.type="competing-risk", 
...
::::::::::::::::::::::::::::::::: dialogue
::: voice-daughter
私「調整前より調整後の累積発生曲線ではわずかに差が広がったみたい。この曲線の差の分だけ、果物摂取の因果効果があるってことだよね」
:::

::: voice-father
お父さん「そういうこと。次に、有効サンプルサイズを見て。調整前の図のアットリスク数より、有効サンプルサイズは小さいでしょ。重みを付けることで誤差が大きくなったったことを示している」
:::
...
::::::::::::::::::::::::::::::::: dialogue
::: voice-father
お父さん「これで最後の作業だ。累積発生曲線のバリデーションをしよう。この場合は、`survfit()`でも累積発生曲線の推定値と標準誤差が計算できる。`survfit()`の推定値は`$pstate`っていうオブジェクトに入っている。`cifplot()`の推定値が欲しければ、計算ルーチンの`cifcurve()`を実行して、`1-$surv`を求めればいい。まったく同じ値になるはずだ。標準誤差は`$std.err`っていうオブジェクトなんだけど、別の計算方法を使っているから近い値になるかを見る。まずは調整なしの結果を比較しよう」
:::
::::::::::::::::::::::::::::::::: 

::: {.callout-note appearance="simple" collapse="true" title="cifcurve()とsurvfit()を比較するためのRコードと結果はこちら"}
```{r warning=FALSE}
...
::: {.callout-note appearance="simple" collapse="true" title="調整累積発生曲線を比較するためのRコードと結果はこちら"}
```{r warning=FALSE}
