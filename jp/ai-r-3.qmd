...
::: {.callout-note appearance="simple" title="図を描く前に決めること"}
調整CIFは、見た目には「きれいに補正された曲線」に見えます。しかし、何を比較するのか、どの集団で比較するのか、どのイベントを関心イベントとし、何を競合リスクとするのかを先に決めておかないと、図の意味は定まりません。

調整済みの曲線や数字は、未調整より「真実に近い数字」と自動的にみなせるものではありません。どの問いに対して、どの集団を想定し、どの仮定の下で、どのモデルを使って答えたのかを示してはじめて、調整後の値として解釈できます。

このページでは、AIにコードを書かせる前に、推定目標、DAG、調整変数、オーバーラップ、共変量バランス、別実装による検証を順番に確認します。AIは作図の近道になりますが、推定目標と仮定を決める近道にはなりません。
:::
...
生存時間・競合リスクデータは2つの変数の組なので、Rの関数に入力するときもペアで指定する必要があります。**survival**パッケージの`survfit()`や`coxph()`では、実は入力専用の関数`Surv()`を用意しています。入力の仕様はパッケージによってまちまちで、たとえば**mets**パッケージでは`Event()`を別に定義して用いています。**cifmodeling**パッケージの`cifplot()`は、`Surv()`と`Event()`の両方に対応しています。

:::
:::

::: {.callout-note appearance="simple" collapse="true" title="R操作で迷ったときの参考資料"}
（競合リスクのない）通常の生存時間解析における`Surv()`、`survfit()`、Kaplan-Meier曲線、Cox回帰の基本操作を確認したい方には、「疫学のためのRハンドブック」の[生存時間解析](https://www.epirhandbook.com/jp/new_pages/survival_analysis.jp.html)も参考になります。
:::
