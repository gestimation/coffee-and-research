...
::: voice-daughter
私「ストーマ造設あり・なしと復職状況を調べる私の調査だったら、仮説の意味から考えても両側p値がおすすめってことね。よく分かりました」
:::
:::::::::::::::::::::::::::::::::
...
::: {.callout-note appearance="simple" title="p値はMethodsと一緒に読む"}
p値を読むときは、数字だけを切り出さず、Methodsに戻ってください。帰無仮説、主要アウトカム、有意水準、片側か両側か、途中解析、多重性への対応が変われば、同じp値でも意味は変わります。

したがって、p=0.04という数字だけで研究の成否を判断するのは危険です。p値は、Methodsに書かれた研究デザインと解析計画の中で意味を持ちます。そのp値が、事前に決めた問い、主要アウトカム、比較、水準、解析手順から出てきたものなのかを確認してはじめて、結果として解釈できます。
:::
...
### 頻度論の確率の捉え方についてのふたりの会話はいかがでしたか？次のシリーズでは、シミュレーションを通じて信頼区間・p値・サンプルサイズが臨床研究でどんな意味を持っているか体験します。

- [Understanding Confidence Intervals via Hypothetical Replications in R](frequentist-4.html)

### シミュレーションはスキップして、その先のエピソードを読みたいという方はこちら。
