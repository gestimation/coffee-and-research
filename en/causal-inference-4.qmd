...
::: {.callout-note appearance="simple" title="Why switch between models?"}
DAGs, probability models, and the Rubin causal model do not describe the same phenomenon in exactly the same language. A DAG helps us reason about connections among variables and candidate adjustment sets. A probability model describes the distribution of observed data and the estimation procedure. The Rubin causal model defines counterfactual questions such as what would have happened under another treatment.

In practice, we first state what we want to know: the estimand. Then we can ask which model supports which part of the argument. Separating those roles makes the purpose of the analysis and its assumptions easier to see.

If we reverse that order and start by putting available variables into a model, the adjusted number can begin to look more authoritative than it really is. An adjusted estimate is not automatically the truth hiding in the data. It is an answer produced by a model for a particular question, target population, and set of assumptions.
:::
