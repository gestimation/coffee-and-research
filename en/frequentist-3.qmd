...
::: {.callout-note appearance="simple" title="Read p-values with the Methods"}
When you read a p-value, do not isolate the number from the rest of the paper. Return to the Methods and check the null hypothesis, primary outcome, significance level, one-sided or two-sided testing, interim analyses, and how multiplicity was handled. If those choices change, the same numerical p-value can mean something different.

That is why it is risky to judge a study simply because p = 0.04. A p-value has meaning within the Methods section and the analysis plan. It becomes interpretable only after you check whether it came from a question, primary outcome, comparison, significance level, and analysis procedure specified in advance.
:::
