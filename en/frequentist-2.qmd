...
::: {.callout-note appearance="simple" title="Common misreadings"}
A p-value is not the probability that the null hypothesis is true. A small p-value does not automatically mean that the result is scientifically or clinically important. A large p-value does not mean "there is no difference," "the treatments are equivalent," or "the study has no value."

When reading a p-value, check at least the null hypothesis, outcome, significance level, test, interim analyses, and multiplicity rules in the Methods. A p-value is not a standalone number; it gets its meaning from the study design and analysis rule.
:::
